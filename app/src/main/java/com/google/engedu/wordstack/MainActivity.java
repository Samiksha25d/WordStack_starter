...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        l1= (LinearLayout) findViewById(R.id.word1);
        l2= (LinearLayout) findViewById(R.id.word2);
        try {
   ...
   /* private class TouchListener implements View.OnTouchListener {
    ...
    public boolean onStartGame(View view) {
        ...
        while(x<Arr1.length || y<Arr2.length)
        {
            ...
            if(random==1 && x<Arr1.length) {
                s1 += Arr1[x];
                x++;
            }
            else if(random==2 && y<Arr2.length) {
                s1 += Arr2[y];
                y++;
            }
        }
        ...
        for(int i=0;i<rev.length();i++) {
            LetterTile letterTile=new LetterTile(getApplicationContext(),rev.charAt(i));
